...
from PySide6.QtGui import QDesktopServices, QGuiApplication, QKeySequence, QRegion
...
class Backend(QObject):
    ...
    def __init__(self, cfg: dict):
        ...
        self._installer_url = ""  # .exe-ассет свежего релиза (Windows auto-update)
        ...
        if tag and _parse_version(tag) > _parse_version(VERSION):
            ...
            for asset in data.get("assets", []):
                if str(asset.get("name", "")).lower().endswith(".exe"):
                    self._installer_url = asset.get("browser_download_url", "")
                    break
    ...
    def applyUpdate(self) -> None:
        """Обновиться прямо из приложения. Windows: тихо переустановить свежий
        installer из ассетов (он сам гасит процесс и перезапускает app). Linux:
        git pull + install.sh в терминале. Фолбэк везде — страница релизов."""
        if _WIN:
            threading.Thread(target=self._update_windows, daemon=True).start()
        else:
            self._update_linux()
    ...
    def _update_windows(self) -> None:
        ...
        if not self._installer_url:
            self._open_releases()
            return
        dst = os.path.join(tempfile.gettempdir(), "VoiceInputSetup.exe")
        try:
            req = urllib.request.Request(self._installer_url, headers={"User-Agent": "voice-input"})
            with urllib.request.urlopen(req, timeout=180) as r, open(dst, "wb") as f:
                shutil.copyfileobj(r, f)
        except Exception:  # noqa: BLE001 — сеть/диск не дали скачать: откроем релизы
            self._open_releases()
            return
        # Тихо: installer сам вызывает --quit и перезапускает app после апдейта.
        # /MERGETASKS=!nvidiagpu — не перекачивать CUDA (~1.3 ГБ) на каждом апдейте,
        # но сохранить прочие задачи по умолчанию (автозапуск и т.п.).
        subprocess.Popen([dst, "/SILENT", "/SUPPRESSMSGBOXES", "/MERGETASKS=!nvidiagpu"])
        self._quitRequested.emit()

    def _update_linux(self) -> None:
        repo = Path(__file__).resolve().parents[1]
        term = self._find_terminal()
        if not (repo / ".git").is_dir() or not (repo / "install.sh").is_file() or not term:
            self._open_releases()  # ставили не из git-клона или нет терминала
            return
        script = (
            f"cd {shlex.quote(str(repo))} && git pull --ff-only && ./install.sh; "
            'echo; read -rp "Обновление завершено. Enter…" _'
        )
        # kitty/foot берут команду напрямую; остальные — через -e.
        direct = os.path.basename(term) in {"kitty", "foot"}
        argv = [term, "bash", "-lc", script] if direct else [term, "-e", "bash", "-lc", script]
        subprocess.Popen(argv, start_new_session=True)

    @staticmethod
    def _find_terminal() -> Optional[str]:
        for name in (os.environ.get("TERMINAL", ""), "kitty", "foot", "alacritty",
