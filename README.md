...
> **Первый запрос / First request:** первая диктовка может обрабатываться
> несколько минут, пока Voice Input один раз скачивает выбранную модель Whisper.
> Не закрывайте приложение. Дальше этой задержки не будет. The first dictation
> may take several minutes while the Whisper model downloads once.
...
После загрузки модель хранится на компьютере: распознавание работает локально,
а аудио никуда не отправляется.
