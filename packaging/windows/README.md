...
```powershell
# GPU-сборка (по умолчанию): вшивает CUDA, включает NVIDIA при device: auto
powershell -ExecutionPolicy Bypass -File packaging\windows\build.ps1

# лёгкая CPU-сборка (без CUDA, ~на 1.5 ГБ меньше)
powershell -ExecutionPolicy Bypass -File packaging\windows\build.ps1 -Gpu:$false
```

Результат: `dist\VoiceInputSetup.exe`.
